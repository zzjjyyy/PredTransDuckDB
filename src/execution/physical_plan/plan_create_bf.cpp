...
PhysicalCreateBF* PhysicalPlanGenerator::CreatePlanfromRelated(LogicalCreateBF &op) {
    if(op.physical == nullptr) {
        ...
        PhysicalCreateBF *create_bf = new PhysicalCreateBF(plan->types, op.bf_to_create_bind, op.estimated_cardinality);
...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePlan(LogicalCreateBF &op) {
    ...
    if(op.physical == nullptr) {
        ...
        create_bf = make_uniq<PhysicalCreateBF>(plan->types, op.bf_to_create_bind, op.estimated_cardinality);
